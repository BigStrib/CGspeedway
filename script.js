...
async function markAttended(id) {
    try {
        ...
        toast('"' + (ev ? ev.event_name : 'Event') + '" attended — added to total', 'success');
...
async function markCancelled(id) {
    try {
        ...
        var ev = null;
        for (var i = 0; i < events.length; i++) { if (events[i].id === id) { ev = events[i]; break; } }
        toast('"' + (ev ? ev.event_name : 'Event') + '" cancelled — not counted', 'info');
    } catch (err) { /* already handled */ }
}
...
function renderDashboard() {
    ...
    summaryEl.innerHTML = ''
        ...
        + '<div class="summary-row"><span class="s-label"><i class="fa-solid fa-circle-xmark" style="color:var(--purple)"></i> Cancelled Event Total</span><span class="s-value" style="color:var(--purple)">$' + totalSaved.toFixed(2) + '</span></div>'
